  ...
  ({ className, type, onChange, inputMode, ...props }, ref) => {
    ...
    return (
      <input
        ...
        className={cn(
          "glass-field h-11 w-full rounded-xl border border-border bg-surface-2 px-3 text-sm font-semibold text-fg outline-none transition-[border-color,box-shadow] duration-150 placeholder:font-medium placeholder:text-faint focus:border-accent focus:shadow-[0_0_0_3px_color-mix(in_srgb,var(--color-accent)_18%,transparent)]",
          className,
        )}
        {...(numeric ? rest : props)}
      />
