...
export function Glance({ size, spec }: { size: GlanceSize; spec: GlanceSpec }) {
  ...
  const headline = (sizes: [number, number, number], emptyText = empty) =>
    hasValue ? (
      ...
      >
        ...
        {unit && <span className="shrink-0 text-[0.45em] font-bold text-faint">{unit}</span>}
  ...
  const gaugeable = progress != null && !visual && !stats;
  ...
  const statGrid = (cols: number, labelled: boolean) =>
        ...
        {stats.map((st) => (
          <div key={st.label} className="min-w-0">
            ...
            <div className="truncate text-[0.78rem] font-extrabold leading-tight tabular" style={{ color: st.color }}>
              {st.value}
              {st.of && <span className="text-[0.62rem] text-faint">{st.of}</span>}
            </div>
          </div>
        ))}
      </div>
  ...
  if (size === "1x1" && gaugeable) {
    ...
    body = (
      <div className="flex h-full min-w-0 items-center gap-3">
        ...
        <div className="flex min-w-0 flex-1 flex-col justify-center gap-1">
          {head}
          {hasValue ? headline([1.4, 1.3, 1.1]) : <div className="truncate text-[0.72rem] font-semibold text-muted">{empty}</div>}
          {sub && <div className="truncate text-[0.62rem] tabular text-faint">{sub}</div>}
        </div>
      </div>
    );
  } else if (size === "1x2") {
    body = (
      <div className="flex h-full min-w-0 items-center gap-3">
        ...
        <div className="flex h-full min-w-0 flex-1 flex-col justify-between gap-1">
          {!visual && head}
          {stats ? (
            <div className="my-auto">{statGrid(2, false)}</div>
          ) : hasValue || !listed.length ? (
            headline([1.6, 1.5, 1.2])
          ) : (
            list(1)
          )}
          {!stats && (bar ?? (sub ? <div className="truncate text-[0.62rem] tabular text-faint">{sub}</div> : null))}
        </div>
      </div>
    );
  } else if (size === "2x2") {
    ...
    body = (
      <div className="flex h-full min-w-0 items-center gap-3">
        ...
        {stats && visual ? (
          <div className="min-w-0 flex-1">{statGrid(4, true)}</div>
        ) : (
          <>
            <div className="flex min-w-0 max-w-[45%] shrink-0 flex-col justify-center gap-1.5">
              {head}
              {hasValue || !listed.length ? headline([1.6, 1.5, 1.25]) : null}
            </div>
            <div className="flex min-w-0 flex-1 flex-col justify-center gap-1.5">
              {hasValue || !listed.length ? (
                <>
                  {sub && <div className="truncate text-[0.68rem] tabular text-muted">{sub}</div>}
                  {!gaugeable && bar}
                  {statGrid(2, true)}
                  {!bar && list(sub ? 1 : 2, true)}
                </>
              ) : (
                list(2, true)
              )}
